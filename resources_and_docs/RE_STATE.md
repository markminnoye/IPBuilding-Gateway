...
- **Relay poll `I<ch>` (2026-06-02, lab):** `I0000`/`I0010`/`I0016`/`I0023` → altijd `I000000000` (10-byte echo, geen `I<CH><state>`). `P0000` → `P000000000` (pulse keep-alive). IPBox idle Run C: hub→relay alleen `pJP0000`, nooit `I<ch>`. **Geen** wijziging `_MODULE_POLL["relay"]`. Status `I<CH><state>` alleen na `S`/`C` (Sprint 1). Evidence: [2026-06-02_relay_poll_i_ch_test.md](evidence/2026-06-02_relay_poll_i_ch_test.md), `scripts/test_relay_poll.py`. **⚠️ ACHTERHAALD 2026-06-12 — verkeerd veldformaat:** de test gebruikte `I00<ch>` (kanaalveld = `"00"`), vandaar de echo. Zie volgende punt.
- **NIEUW: cold-boot relay status-sweep (2026-06-12):** bij koude start leest de IPBox de stand van elke geconfigureerde relay-uitgang via `I<CH>00` → `I000<CH><state>` (`0100`=AAN, `0000`=UIT), één query per kanaal (~90 ms apart), daarna pas `P0000`-keepalive. Gevangen op **relay-poort** in `captures/2026-06-12_IPBOX_BOOT.pcapng`: kanalen 17–23 (18 & 23 AAN). **Correctie op 2026-06-02:** kanaal staat in de **eerste** twee cijfers (`I<CH>00`), niet de laatste (`I00<CH>`) — daarom kreeg de lab-test steeds kanaal 00. Relay ondersteunt dus wél een on-demand status-read; gateway zou bij startup deze sweep moeten doen over geconfigureerde outputs (`gateway/udp_bus.py:20-21` comment achterhaald; decoder `payloads/relay.py` `relay_status` ondersteunt de reply al). Evidence: [2026-06-12_ipbox_boot_relay_sweep.md](evidence/2026-06-12_ipbox_boot_relay_sweep.md).
- **Relay state_code buiten `0000`/`0100` (2026-08-08, Nolf; mapped 2026-08-23):** startup-poll op oudere IP0200 `.30` levert **`0015`** (prefix `00` → off) en één× **`0115`** (prefix `01` → on, toilet). Lab-quartets `0000`/`0100` ongewijzigd. Decoder (gateway 1.6.4 + ESP32) mapt `00xx`/`01xx`; **fysieke AAN/UIT bij Jan nog te bevestigen**. Nolf command-replies gebruiken wél lab-codes `0100`/`0000`. Evidence: [2026-08-08_jan_nolf_restore_test.md](evidence/2026-08-08_jan_nolf_restore_test.md) §5, state_code-tabel in [2026-05-04_relay_payload_correlation.md](evidence/2026-05-04_relay_payload_correlation.md).
- **IPBox-identiteit + dual-NIC + shutdown (2026-06-12):** IPBox = één Windows-10 embedded-PC, hostname **`IP2017-814`** (Jetway OUI `00:30:18`), twee NIC's die beide die naam registreren: `192.168.0.185` (module-polling + Azure-cloud + No-IP dyn-DNS) en `10.10.1.1` (IPBuilding-VLAN, Windows-discovery). `.185` probeert intern continu TCP naar `10.10.1.1:1024/1025` (vermoedelijk config-sync-kanaal; in capture host-unreachable). **Shutdown:** geen goodbye-bericht; polling stopt abrupt. Steady-state poll bevestigd voor alle 3 modules: input `I0000` ~2 s, dimmer `I9900` ~20 s, relay `P0000` ~20 s. Evidence: [2026-06-12_ipbox_boot_relay_sweep.md](evidence/2026-06-12_ipbox_boot_relay_sweep.md), `captures/2026-06-12_IPBOX_POWER_ON_AND_OFF.pcapng`.
- **Dimmer heeft GEEN on-demand statuspoll via `I<CH>00` / `I9900` (2026-06-12):** **H1/H2 verworpen** voor die formaten. **⚠️ GEDEELTELIJK ACHTERHAALD 2026-08-05:** er *is* een per-kanaal statusquery — formaat **`I{ch}000000`** (8 bytes) → `I0154{ch}{vv}`. Evidence update: [2026-08-05_dimmer_I_ch_000000_status_poll.md](evidence/2026-08-05_dimmer_I_ch_000000_status_poll.md). Oorspronkelijke H1/H2-test: [2026-06-12_dimmer_status_poll_hypotheses_refuted.md](evidence/2026-06-12_dimmer_status_poll_hypotheses_refuted.md).
- **Dimmer on-demand statuspoll `I{ch}000000` CONFIRMED (2026-08-05):** live op lab `.40`; ch2=100% vs overige off bewijst kanaalselectie. Nolf UI Actions matchen dit formaat. **Gateway:** `encode_dimmer_status_poll` + `sweep_dimmer_states` (startup / post-discovery). Evidence: [2026-08-05_dimmer_I_ch_000000_status_poll.md](evidence/2026-08-05_dimmer_I_ch_000000_status_poll.md).
- **Input→dimmer peer-to-peer pad (2026-06-22, dimmer-poort capture):** IP1100PoE (`10.10.1.50`) stuurt commando's **rechtstreeks** naar IP0300PoE (`10.10.1.40`) — hub `10.10.1.1` afwezig op dit pad. **Sluit de "Autonomous mode" hypothese** uit `Unknowns/open hypotheses`: dit is het reguliere schakelpad tijdens normaal IPBox-bedrijf, niet een noodmodus. Nieuw command-vocabulaire (input-module dialect, **twee dialecten op UDP/1001**): `T<ch><vv>1000` = toggle, `D<ch><vv>1003` = dim hold start (auto-richting, geen ack!), `D<ch><vv>1000` = dim hold stop (ack `I0154<ch><vv>`). Hold = **start/stop protocol**: dimmer dimmt autonoom tussen de twee pakketten (~31%/s up, ~20%/s down); identieke wire-code voor dim-up en dim-down (IP0300 wisselt intern van richting). Hub-dialect ongewijzigd: `S<ch><vv>1030` / `C<ch>991030`. Evidence: [2026-06-22_dimmer_p2p_hold_dim_capture.md](evidence/2026-06-22_dimmer_p2p_hold_dim_capture.md), `/Users/markminnoye/Downloads/capture dimmer.pcapng` (12 frames, POV dimmer-poort).
...
- ~~**Autonomous mode:** input→relay/dimmer direct pad niet gecaptured (centrale uit, LED knipperend).~~ **GESLOTEN 2026-06-22:** input→dimmer peer-to-peer pad bevestigd in `capture dimmer.pcapng`; dit is het **reguliere schakelpad** (niet een noodmodus). Zie [2026-06-22_dimmer_p2p_hold_dim_capture.md](evidence/2026-06-22_dimmer_p2p_hold_dim_capture.md).
...
- `/Users/markminnoye/Downloads/capture dimmer.pcapng` (2026-06-22 dimmer P2P; POV dimmer-poort; 12 frames; input→dimmer `T/D1003/D1000` + hold-dim start/stop protocol)
