...
def func_to_target_entity(
    ...
    name_slug = channels.get(  # type: ignore[arg-type]
        int(ch) if str(ch).isdigit() else 0, Channel(0, "", "", "", out_type)
    ).name
    if out_type == "relay":
        entity = f"light.{name_slug.lower()}" if name_slug else f"switch.{module_ip}_{ch}"
    elif out_type == "dimmer":
        entity = f"light.{name_slug.lower()}" if name_slug else f"light.{module_ip}_{ch}"
    elif out_type == "motion":
        entity = f"binary_sensor.{name_slug.lower()}" if name_slug else None
        notes.append("motion: geen dim-loop; alleen press-toggle")
    else:
        entity = f"switch.{name_slug.lower()}" if name_slug else None
        notes.append(f"onbekende outType {out_type!r}")
...
def render_automations_yaml(
    ...
    for entry in entries:
        ...
        if entry.func1_action:
            ...
            lines.append("      event_type: ipbuilding_gateway_ha.button_pressed")
        ...
        if entry.func2_action:
            ...
            lines.append("  triggers:")
            lines.append("    - trigger: event")
            lines.append("      event_type: ipbuilding_gateway_ha.button_long_pressed")
            lines.append("      event_data:")
            lines.append(f'        hardware_id: "{btn.id}"')
            lines.append("  actions:")
...
def import_buttons(
    ...
    for btn in buttons:
        ...
        if btn.func1 and not target1:
            warnings.append(
                f"Knop {friendly_name!r} ({btn.id}): func1 heeft geen herleidbare target — overgeslagen"
            )
            continue
...
def _existing_helpers(out_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    if not path.exists():
        return {}
    try:
        data = yaml_or_json_load(path)
    except Exception:
        return {}
    return data.get("input_boolean", {}) if isinstance(data, dict) else {}


def yaml_or_json_load(text: str) -> Any:
    """Load a file as YAML (preferred) or JSON (fallback)."""
    try:
