...
from gateway.installation import ChannelConfig, InstallationConfig, InstallationError, ModuleConfig
        ...
        "modules": [
            {
                ...
                "type": "relay",
                "channels": [
                    {"ch": 0, "id": 547, "description": "Keuken LED", "group": "Keuken"},
                    {"ch": 10, "id": 557, "description": "Patio", "group": "Buitenverlichting"},
                    {"ch": 16, "id": 563, "description": "Keuken LED 2"},
                    {"ch": 23, "id": 570, "description": "Keuken Ventilatie"},
                ],
            },
            {
                "name": "dimmer_module",
                ...
                "type": "dimmer",
                "channels": [
                    {"ch": 0, "id": 571, "description": "Woonkamer Dimmer 1"},
                    {"ch": 1, "id": 572, "description": "Woonkamer Dimmer 2"},
                ],
            },
            {
                "name": "input_module",
...
class TestInstallationLoad:
    def test_load_valid(self, valid_devices_json: Path) -> None:
        ...
        assert relay.channels[0].id == 547
    ...
    def test_id_to_channel(self, valid_devices_json: Path) -> None:
        cfg = InstallationConfig.load(valid_devices_json)

        entry = cfg.id_to_channel(547)
        assert entry == (DeviceType.RELAY, "10.10.1.30", 0)

        entry = cfg.id_to_channel(571)
        assert entry == (DeviceType.DIMMER, "10.10.1.40", 0)

        assert cfg.id_to_channel(9999) is None
    ...
    def test_all_component_ids(self, valid_devices_json: Path) -> None:
        cfg = InstallationConfig.load(valid_devices_json)
        ids = cfg.all_component_ids()
    ...
    def test_duplicate_component_id(self, tmp_path: Path) -> None:
        data = {
            "modules": [
                {"ip": "10.10.1.30", "type": "relay", "channels": [{"ch": 0, "id": 547}]},
                {"ip": "10.10.1.40", "type": "dimmer", "channels": [{"ch": 0, "id": 547}]},
            ]
        }
        p = tmp_path / "dup.json"
        p.write_text(json.dumps(data), encoding="utf-8")
    ...
    def test_channel_missing_fields(self, tmp_path: Path) -> None:
        data = {
            "modules": [
                {"ip": "10.10.1.30", "type": "relay", "channels": [{"ch": "not_an_int", "id": 547}]}
            ]
        }
        p = tmp_path / "bad_ch.json"
        p.write_text(json.dumps(data), encoding="utf-8")
        with pytest.raises(InstallationError, match="missing 'ch' or 'id'"):
            InstallationConfig.load(p)
