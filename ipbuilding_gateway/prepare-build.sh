...
VERSION=$(grep -E '^version:' "$CONFIG_YAML" | head -1 | sed -E 's/^version:[[:space:]]*"?([^"]+)"?/\1/')
echo "[prepare-build] Embedding version $VERSION from config.yaml into gateway/_version.py"
cat > "$REPO_ROOT/ipbuilding_gateway/gateway/_version.py" <<EOF
# Auto-generated by prepare-build.sh — do not edit.
# Source of truth: ipbuilding_gateway/config.yaml
__version__ = "$VERSION"
EOF
